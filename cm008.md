...
```r
ggplot(mpg, aes(cty, hwy)) + # ggplot, must be a plus  
  geom_jitter(alpha = 0.5, size = 1) + #points are coarsley rounded, could do jitter ,  
  #change to jitter, points are jittered randomly 
  # could do alpha transparancy and change the point size to fix overplot 
  geom_smooth(method = "lm") +  #not a straight line, lm (linear model in R), not the quote
  # geom_jitter does not change the original data, "lm" fits to the original data 
  # could even delete the geom_jitter, just get a line 
  theme_bw()
```
...
```r
gapminder %>% 
  # group_by(country) %>% # ggplot is ignoring the grouping factor, grouping in dplyr is different 
  ggplot(aes(year, lifeExp, group = country, colour = country == "Rwanada")) + #now ggplot is chunking the tibble 
  geom_line(alpha = 0.2) +
  scale_colour_discrete (" ", labels = c("Other", "Rwanada"))
```

![](cm008-exercise_files/figure-html/unnamed-chunk-3-1.png)<!-- -->

```r
# not trying to pick out a country, try and color, not enough colours and legend is too big, make an interacive plot so you can mouse over 
  # columns created on the fly, '=' indicating
```
...
Sci.Not not that great
labels arguments changes aest
$_format, change axis to have dollar quantities 
...
ggplot(gapminder, aes(gdpPercap, lifeExp)) +
  ...
  # would want to chage the x and y labels gdppercap --> GDP per Capita annd
  # spread out the data: use faceting 
  # facet_wrap(~ continent) +
  # axis are shared in order to more easily compare each plot 
  # facet_wrap(~continent, scales = "free") +
  # can no longer compare 
  # can also specify which axis you want to free up 
...
- Area of points correspond to pop -- > add new scale
- overplotting, alpha!! 
...
gapminder %>% 
  ...
  #filter out Ocenia; not much data
  # ggplot(aes(gdpPercap, lifeExp)) +
  # ggplot(aes(gdpPercap, lifeExp, size = pop, colour = continent)) colour identifies perimeter of object but fill specifies inside colour 
  # ggplot(aes(gdpPercap, lifeExp, size = pop, colour = continent)) 
  ggplot(aes(gdpPercap, lifeExp, size = pop, fill = continent)) +
  # facet_wrap(~ continent) +
  facet_wrap(~ continent, nrow = 1) + 
  # geom_point(alpha = 0., shape = 21)
  geom_point(alpha = 0.5, shape = 21) + # 21 = "hollow" circle" 
  scale_x_log10(labels = scales::comma_format()) + 
  # population is reflected in point area: 
  scale_size_area() # solves overlpotting too
```
...
** Think that this is where we stopped in class** 
