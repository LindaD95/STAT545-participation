...
read_csv("https://raw.githubusercontent.com/STAT545-UBC/STAT545-home/master/content/songs.csv")
#needed to install readr and correct songs
...
```{r}
album %>%
  full_join(album, by = c("song", "artis_name"))
```
...
fell %>% 
  ...
  anti_join(retk, by = "Race") # by = "Race") 
```
...
```{r}
(y <-  tibble(x1 = LETTERS[1:3], x2 = 1:3)) # view as paired observations 
```
...
Same column names in both cases -> intersect y and x

```{r}
intersect(y, x)
# inner_join will also work 
```
...
FILL_THIS_IN(
  mutate(y, day = "Day 1"),
  mutate(z, day = "Day 2")
  # make into one unifed tibble: union,
)
```
...
```{r}
setdiff(y, z) # the one removed should be at the end 
