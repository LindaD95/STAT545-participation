...
Plot the residuals for this data

```{r}
sim1_lm <- augment(my_lm)
sim1_lm %>% 
  ggplot(aes(.resid)) +
...
predict(my_lm, newdata = gapminder_France2) %>%
  head()
# process a subset of data without using the whole linear regression again 
```

```{r}
plot(my_lm)
# various forms of data and residual
...
# do not model as linear; or use a linear model in a different way 
```
Let's try fitting a linear model to this relationship
```{r incorrect}
ggplot(gapminder_Zimbabwe, aes(year,lifeExp)) + 
  geom_point()+
  geom_smooth(method = "lm", formula = lifeExp ~ poly(I(year - 1952), degree = 2))
```
