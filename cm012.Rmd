...
gapminder %>% 
  ...
  ggplot() + geom_boxplot(aes(x = life_level, y = gdpPercap)) +
  labs(y = "GDP per capita, $", x= "Life expectancy level, years") +
  scale_x_discrete(drop = FALSE) + # no observation in the column; dropped by default but still want to keep; visually not effective to compare the levels if they are all there 
  theme_bw() 
#life_level is not a factor 
...
```{r}
gapminder$continent %>%  # stored as a factor 
  str() # could also run run class() 
```
...
gapminder$continent %>% 
  levels() %>% 
# factros are stored as integers; determine the integers, run str()
str()
...
```{r}
gapminder %>% # instead of using
  filter(country == "Canada" | country == "Mexico" | country ) 
# choose countries with factors
```
...
h_countries= c("Austria", "Germany", "France", "Slovakia", "Hungary", "Norway")
gap= gapminder %>% 
...
gapminder %>% 
  count(continent) 
# convert to graph, can just use geom_bar ()
...
gapminder %>%  
  ggplot() +
  geom_bar(aes(continent)) + # default is count fo the y
  coord_flip() +
  theme_bw() +
...
gapminder %>%  
  ggplot() +
  geom_bar(aes(fct_infreq(continent))) + # default is count fo the y, sort by the count
  coord_flip() +
  theme_bw() +
  ylab("Number of entries") + xlab("Continent")
...
gapminder %>%  
  ggplot() +
  geom_bar(aes(fct_infreq(continent, lifeExp, min))) + # default is count fo the y, sort by minimum lifeExp ; think that this miight the wrong function 
  coord_flip() +
  theme_bw() +
  ylab("Number of entries") + xlab("Continent")
```
...
gapminder %>%  
  ...
  # do not nest together a relevel and recode; bad style 
  coord_flip() +
  theme_bw() +
  ylab("Number of entries") + xlab("Continent")
```
