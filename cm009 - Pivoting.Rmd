...
```{r}
lotr
# looks pretty tidy but whether or not data is tidy it is relative 
```
...
```{r}
(lotr_wide <- lotr %>% 
  pivot_wider(id_cols = c(-Race, -Words), # not wanting race or a words column as an identifer
              names_from = Race, 
              values_from = Words)) 

# could then rearrange the order of things using arrange()  
```
...
```{r}
lotr_wide %>% 
  pivot_longer(cols = c(-Film, -Gender), # first idicate the columns you want to work with
               names_to  = "Race", 
               values_to = "words")
```
...
```{r}
(guest_long <- guest %>% 
  pivot_longer(cols = c(-party, -name), # working with everything except party and name
               names_to  = c(".value", "event"),  # left side.... retaining names
               names_sep = "_")) # what side of the column (that you are working with) that will retain its names and wich will
```
...
```{r}
guest_long %>% 
  pivot_wider(id_cols = c(part, name), 
              names_from = event, 
              names_sep   = "_", 
...
```{r}
email %>% 
  separate_rows(guest, sep = ", ") # note the spacing of ", " and not " , " 
```
...
```{r}
n <- 100
# missed this,, was trying to load data and packages from cm010)
```
