...
1. Colours 
2. Showing all the data (jitter plots versus bar graphs with error bars)
3. Weighted points and highlighting min and max
4. 3D Plots
5. Faceting 
...
- Weak due crappy wording of questions, no measure of standard errors, sample size, what industries are the samples, axis adjustment
...
> 
> Do not know; what is meant by appreciable 

- Garabge, WHAT IS THE X AXIS, interesting quib at the use of farhenheit, no data points (is a continuous line the best choice) 
...
- Hate the red, inverted y axis very misleading
...
# plot_ly Syntax; should get the same output as above
    ...
    #      y = ~lifeExp, 
     #     colour = ~continent, 
      #    type = 'scatter', 
       #   mode = 'scatter') + 
      # theme_bw() 
        # ~ denotes it as a mapping variable
```

Make an account on plot.ly type in code chunk
#```{r}
sys.sentenc(username...
```

```{r Upload Interactive plot to Plotly} 
